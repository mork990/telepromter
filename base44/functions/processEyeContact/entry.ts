...
function buildProtoDefinitions() {
  ...
  return { root, RedirectGazeRequest, RedirectGazeResponse };
}
...
function createGrpcServiceClient(proto) {
  ...
  return grpc.makeClientConstructor(serviceDefinition, 'MaxineEyeContactService');
}
  ...
  return new Promise((resolve, reject) => {
    ...
    call.on('end', () => {
      ...
      const uint8Chunks = outputChunks.map(chunk => {
        if (chunk instanceof Uint8Array && chunk.constructor === Uint8Array) return chunk;
        if (Buffer.isBuffer(chunk)) return new Uint8Array(chunk.buffer, chunk.byteOffset, chunk.byteLength);
        if (chunk.buffer && chunk.byteOffset !== undefined) {
          return new Uint8Array(chunk.buffer, chunk.byteOffset, chunk.byteLength);
        }
        return new Uint8Array(chunk);
      });
      const totalLength = uint8Chunks.reduce((acc, c) => acc + c.byteLength, 0);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('processEyeContact started for:', recording_id);
    ...
    console.log('Starting NVIDIA processing...');
    ...
    console.log('Upload done:', uploadResult.file_url?.substring(0, 80));
    ...
    console.error('processEyeContact error:', error.message);
    if (base44 && recording_id) {
      try {
        await base44.asServiceRole.entities.Recording.update(recording_id, {
